...
用戶的操作都在 LINE：自然提問 → 判斷問題主題 → 顯示對應教學。入金教學每次一張圖配一段說明；沒有截圖的確認步驟提供文字卡片。已移除「文字就好」模式。後台網站供管理員管理客戶、知識庫、人工協助案件與測試對話。
...
BitoPro 入金共 14 步（12 張圖片、2 步文字確認），信用卡入金共 8 步。LINE 與網站共用 `public/guide-bitopro.js`、`public/guide-card.js` 的逐張說明，每次只回覆一張 Flex 卡片，顯示目前步數、圖片與對應文字。圖片使用 fit 保留完整畫面，可點開放大。卡片內保留「下一張」「上一張」「選擇入金方式」「人工協助」「選單」，最後一步提供「提交 UID」。舊卡片的按鈕仍指向指定主題與頁碼；口語「下一張／上一張」依目前上下文切換。註冊、邀請碼、KYC 和 UID 的最後一則訊息也有固定導覽卡片。快速回覆附在最後一則訊息，但 LINE 收到新訊息時仍可能隱藏，因此不把它當成唯一操作入口。用戶主動上傳的截圖仍交由人工查看，尚未接入視覺辨識。
...
- 在 LINE 分別開啟 BitoPro 與信用卡入金，確認每次僅一張對應圖片、短說明與步數。
- 逐張前進、返回；完成後再按上一張，確認不會卡在最後一步。切換教學後點舊卡片，確認仍開啟該卡指定步驟。
- 傳送其他訊息讓快速回覆收起，再回頭點卡片內按鈕，確認仍可操作。
- 未提交 UID 時查詢進度，應提供 UID 提交入口；客服接手後再次要求人工協助，應可點「繼續使用小幫手」。
- 本機測試及 dry-run 不代表 LINE 手機實機驗收或已部署。
